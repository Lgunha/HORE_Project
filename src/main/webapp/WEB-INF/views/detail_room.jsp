			...
			<div class="col-md-8">
				...
				<table class="table-dark table table-striped" border=1
					...
					<c:if test="${nowUser == id_hotel}">
						<tr class="table-success">
							<td colspan=4><a
								href="update_room?detailAddr_roomNum_room=${roomvo.detailAddr_roomNum_room }"
								class="btn btn-outline-primary ">방 수정</a> <a
								href="delete_room?detailAddr_roomNum_room=${roomvo.detailAddr_roomNum_room }"
								class="btn btn-outline-primary ">방 삭제</a></td>
						</tr>
					</c:if>
					<c:if test="${nowUser !='비회원'}">
						<c:if test="${nowUser != id_hotel}">
							<c:if test="${checkIn > sysdate}">
							<tr class="table-success">
								<td colspan=4>
								<a href="reservation?detailAddr_roomNum_room=${roomvo.detailAddr_roomNum_room }&checkIn=${checkIn}&checkOut=${checkOut}" class="btn btn-outline-primary ">방 예약</a>
								</td>
							</tr>
							</c:if>
						</c:if>
